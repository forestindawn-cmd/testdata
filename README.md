...
- **현재 위치 날씨**: 🚀 GPS 기반 현재 위치 자동 감지 및 날씨 표시
...
├── location_service.py # 🚀 GPS 위치 서비스 및 Geolocation API 모듈
├── config.py          # 설정 파일
...
### `location_service.py` 🚀
- 브라우저 Geolocation API 인터페이스
- JavaScript와 Streamlit 간 GPS 좌표 통신
- 위치 정보 캐싱 및 보안 처리
- HTML/JavaScript 컴포넌트 렌더링
- 실시간 위치 업데이트 및 오류 처리
...
1. **지역 검색 방법 선택**:
   ...
   - **인기 지역 선택**: 버튼으로 빠른 선택
   - **📍 현재 위치**: GPS 기반 현재 위치 자동 감지

2. **현재 위치 사용법**: 🚀
   - "📍 현재 위치" 라디오 버튼 선택
   - "현재 위치 날씨 보기" 버튼 클릭
   - 브라우저에서 위치 접근 허용
   - 자동으로 GPS 좌표 기반 날씨 표시
...
5. **자동완성 활용**: 입력 중 나타나는 검색 제안을 클릭

6. **실시간 정보 확인**: 현재 날씨와 5일 예보 확인

### 📍 현재 위치 기능 특징

- **정확한 GPS 좌표**: 브라우저 Geolocation API 사용
- **자동 위치 감지**: 버튼 클릭 한 번으로 현재 위치 날씨 확인  
- **실시간 좌표 표시**: 위도/경도 정보 함께 표시
- **위치 정보 캐싱**: 10분간 위치 정보 저장으로 재사용 가능
- **보안 고려**: HTTPS 환경에서만 정상 작동
...
### 개인정보 보호
- **위치 정보는 로컬에서만 처리**되며 서버에 저장되지 않습니다
- **브라우저의 로컬 스토리지**에 10분간만 임시 저장됩니다
- **HTTPS 연결**에서만 위치 서비스가 정상 작동합니다

### 기술적 제한사항
- **실내에서는 정확도가 떨어질 수 있습니다**
- **VPN 사용시 위치가 부정확할 수 있습니다**
- **모바일 기기에서 더 정확한 위치 제공**
- **위치 서비스가 비활성화된 경우 작동하지 않습니다**
...
- [Geolocation API](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API) - 브라우저 위치 서비스